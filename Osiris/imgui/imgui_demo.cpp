...
// will want to refer to and call. Have the ui::ShowDemoWindow() function wired in an always-available debug menu of
// your game/app! Removing this file from your project is hindering access to documentation for everyone in your team,
// likely leading you to poorer usage of the library.
// Everything in this file will be stripped out by the linker if you don't call ui::ShowDemoWindow().
...
void ui::ShowDemoWindow(bool* p_open)
{
    ...
    //ui::PushItemWidth(ui::GetWindowWidth() * 0.65f);    // Use 2/3 of the space for widgets and 1/3 for labels (default)
    ...
    if (ui::CollapsingHeader("Configuration"))
    {
        ...
        if (ui::TreeNode("Configuration##2"))
        {
            ...
            ui::Checkbox("io.ConfigInputTextCursorBlink", &io.ConfigInputTextCursorBlink, 0);
            ui::SameLine(); ShowHelpMarker("Set to false to disable blinking cursor, for users who consider it distracting");
            ui::Checkbox("io.ConfigWindowsResizeFromEdges", &io.ConfigWindowsResizeFromEdges, 0);
            ...
            ui::Checkbox("io.ConfigWindowsMoveFromTitleBarOnly", &io.ConfigWindowsMoveFromTitleBarOnly, 0);
            ui::Checkbox("io.MouseDrawCursor", &io.MouseDrawCursor, 0);
        ...
        if (ui::TreeNode("Capture/Logging"))
        {
            ...
            ui::LogButtons();
            ui::TextWrapped("You can also call ui::LogText() to output directly to the log without a visual output.");
    ...
    {
        ui::Checkbox("No titlebar", &no_titlebar, 0); ui::SameLine(150);
        ui::Checkbox("No scrollbar", &no_scrollbar, 0); ui::SameLine(300);
        ui::Checkbox("No menu", &no_menu, 0);
        ui::Checkbox("No move", &no_move, 0); ui::SameLine(150);
        ui::Checkbox("No resize", &no_resize, 0); ui::SameLine(300);
        ui::Checkbox("No collapse", &no_collapse, 0);
        ui::Checkbox("No close", &no_close, 0); ui::SameLine(150);
        ui::Checkbox("No nav", &no_nav, 0), 0; ui::SameLine(300);
        ui::Checkbox("No background", &no_background, 0);
        ui::Checkbox("No bring to front", &no_bring_to_front, 0);
    }
...
static void ShowDemoWindowWidgets()
{
    ...
    if (ui::TreeNode("Basic"))
    {
...

        {
            ...
            //ui::PushItemWidth(-1);
            //ui::ListBox("##listbox2", &listbox_item_current2, listbox_items, IM_ARRAYSIZE(listbox_items), 4);
            //ui::PopItemWidth();
        }
    ...
    //for (int i = 0; i < 5; i++)
    //    if (once)
    //        ui::Text("This will be displayed only once.");
    ...
    if (ui::TreeNode("Trees"))
    {
        ...
        if (ui::TreeNode("Advanced, with Selectable nodes"))
        {
            ...
            ui::Checkbox("Align label with current X position)", &align_label_with_current_x_position, 0);
            ui::Text("Hello!");
    ...
    if (ui::TreeNode("Collapsing Headers"))
    {
        ...
        ui::Checkbox("Enable extra group", &closable_group, 0);
    ...
    if (ui::TreeNode("Images"))
    {
        ...
        // If you decided that ImTextureID = MyEngineTexture*, then you can pass your MyEngineTexture* pointers to ui::Image(), and gather width/height through your own functions, etc.
    ...
    if (ui::TreeNode("Multi-line Text Input"))
    {
        ...
        ui::Checkbox("Read-only", &read_only, 0);
    ...
    if (ui::TreeNode("Color/Picker Widgets"))
    {
        ...
        ui::Checkbox("With Alpha Preview", &alpha_preview, 0);
        ui::Checkbox("With Half Alpha Preview", &alpha_half_preview, 0);
        ui::Checkbox("With Drag and Drop", &drag_and_drop, 0);
        ui::Checkbox("With Options Menu", &options_menu, 0); ui::SameLine(); ShowHelpMarker("Right-click on the individual color widget to show options.");
        ui::Checkbox("With HDR", &hdr, 0); ui::SameLine(); ShowHelpMarker("Currently all this does is to lift the 0..1 limits on dragging widgets.");
        ...
        ui::Checkbox("With Alpha", &alpha, 0);
        ui::Checkbox("With Alpha Bar", &alpha_bar, 0);
        ui::Checkbox("With Side Preview", &side_preview, 0);
        if (side_preview)
        {
            ui::SameLine();
            ui::Checkbox("With Ref Color", &ref_color, 0);
            if (ref_color)
            {
                ui::SameLine();
    ...
    if (ui::TreeNode("Data Types"))
    {
        ...
        ui::Text("Drags:");
        ui::Checkbox("Clamp integers to 0..50", &drag_clamp, 0); ui::SameLine(); ShowHelpMarker("As with every widgets in dear imgui, we never modify values unless there is a user interaction.\nYou can override the clamping limits by using CTRL+Click to input a value.");
        ...
        ui::Text("Inputs");
        ui::Checkbox("Show step buttons", &inputs_step, 0);
    ...
    if (ui::TreeNode("Querying Status (Active/Focused/Hovered etc.)"))
    {
        ...
        if (item_type == 2) { ret = ui::Checkbox("ITEM: Checkbox", &b, 0); }                            // Testing checkbox
        if (item_type == 3) { ret = ui::SliderFloat("ITEM: SliderFloat", &col4f[0], 0.0f, 1.0f); }   // Testing basic item
        if (item_type == 4) { ret = ui::ColorEdit4("ITEM: ColorEdit4", col4f, 0); }                     // Testing multi-component items (IsItemXXX flags are reported merged)
        ...
        ui::Checkbox("Embed everything inside a child window (for additional testing)", &embed_all_inside_a_child_window, 0);
        ...
        ui::Checkbox("Hovered/Active tests after Begin() for title bar testing", &test_window, 0);
        if (test_window)
        {
...
static void ShowDemoWindowLayout()
{
    ...
    if (ui::TreeNode("Child windows"))
    {
        ...
        ui::Checkbox("Disable Mouse Wheel", &disable_mouse_wheel, 0);
        ui::Checkbox("Disable Menu", &disable_menu, 0);
        ...
        // - Using ui::GetItemRectMin/Max() to query the "item" state (because the child window is an item from the POV of the parent window)
    ...
    if (ui::TreeNode("Basic Horizontal Layout"))
    {
        ...
        ui::Checkbox("My", &c1, 0); ui::SameLine();
        ui::Checkbox("Tailor", &c2, 0); ui::SameLine();
        ui::Checkbox("Is", &c3, 0); ui::SameLine();
        ui::Checkbox("Rich", &c4, 0);
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            ui::PopID();
            //if (ui::IsItemHovered()) ui::SetTooltip("ListBox %d hovered", i);
        }
        ui::PopItemWidth();
    ...
    if (ui::TreeNode("Tabs"))
    {
        ...
        if (ui::TreeNode("Advanced & Close Button"))
        {
            ...
            {
                if (n > 0) { ui::SameLine(); }
                ui::Checkbox(names[n], &opened[n], 0);
            }
    ...
    {
        ShowHelpMarker("Using ui::BeginGroup()/EndGroup() to layout items. BeginGroup() basically locks the horizontal position. EndGroup() bundles the whole group so that you can use functions such as IsItemHovered() on it.");
        ui::BeginGroup();
        {
            ui::BeginGroup();
            ui::Button("AAA");
...
static void ShowDemoWindowPopups()
{
    ...
    if (ui::TreeNode("Modals"))
    {
        ...
        if (ui::BeginPopupModal("Delete?", NULL, ImGuiWindowFlags_AlwaysAutoResize))
        {
            ...
            //ui::Combo("Combo", &dummy_i, "Delete\0Delete harder\0");

            static bool dont_ask_me_next_time = false;
            ui::PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2(0, 0));
            ui::Checkbox("Don't ask me next time", &dont_ask_me_next_time, 0);
            ui::PopStyleVar();
...
static void ShowDemoWindowColumns()
{
    ...
    if (ui::TreeNode("Borders"))
    {
        ...
        ui::Checkbox("horizontal", &h_borders, 0);
        ui::SameLine();
        ui::Checkbox("vertical", &v_borders, 0);
    ...
    // Scrolling columns
    /*
    if (ui::TreeNode("Vertical Scrolling"))
    {
        ui::BeginChild("##header", ImVec2(0, ui::GetTextLineHeightWithSpacing()+ui::GetStyle().ItemSpacing.y));
        ui::Columns(3);
        ui::Text("ID"); ui::NextColumn();
        ui::Text("Name"); ui::NextColumn();
        ui::Text("Path"); ui::NextColumn();
        ...
        ui::Separator();
        ui::EndChild();
        ui::BeginChild("##scrollingregion", ImVec2(0, 60));
        ui::Columns(3);
        for (int i = 0; i < 10; i++)
        {
            ui::Text("%04d", i); ui::NextColumn();
            ui::Text("Foobar"); ui::NextColumn();
            ui::Text("/path/foobar/%04d/", i); ui::NextColumn();
        }
        ui::Columns(1);
        ui::EndChild();
        ui::TreePop();
...
static void ShowDemoWindowMisc()
{
    ...
    if (ui::CollapsingHeader("Inputs, Navigation & Focus"))
    {
        ...
        if (ui::TreeNode("Tabbing"))
        {
            ...
            //ui::SameLine(); ShowHelperMarker("Use ui::PushAllowKeyboardFocus(bool)\nto disable tabbing through certain widgets.");
        ...
        if (ui::TreeNode("Mouse cursors"))
        {
            ...
            ui::SameLine(); ShowHelpMarker("Your application can render a different mouse cursor based on what ui::GetMouseCursor() returns. If software cursor rendering (io.MouseDrawCursor) is set ImGui will draw the right cursor for you, otherwise your backend needs to handle it.");
...
void ui::ShowAboutWindow(bool* p_open)
{
    ...
    ui::Checkbox("Config/Build Information", &show_config_info, 0);
    if (show_config_info)
    {
        ImGuiIO& io = ui::GetIO();
        ImGuiStyle& style = ui::GetStyle();
        ...
        ui::Text("define: IMGUI_DISABLE_OBSOLETE_FUNCTIONS");
#endif
        ...
        ui::Text("define: IMGUI_DISABLE_FORMAT_STRING_FUNCTIONS");
#endif
#ifdef IMGUI_DISABLE_MATH_FUNCTIONS
        ui::Text("define: IMGUI_DISABLE_MATH_FUNCTIONS");
#endif
...
void ui::ShowStyleEditor(ImGuiStyle* ref)
{
    ...
    { bool window_border = (style.WindowBorderSize > 0.0f); if (ui::Checkbox("WindowBorder", &window_border, 0)) style.WindowBorderSize = window_border ? 1.0f : 0.0f; }
    ui::SameLine();
    { bool frame_border = (style.FrameBorderSize > 0.0f); if (ui::Checkbox("FrameBorder", &frame_border, 0)) style.FrameBorderSize = frame_border ? 1.0f : 0.0f; }
    ui::SameLine();
    { bool popup_border = (style.PopupBorderSize > 0.0f); if (ui::Checkbox("PopupBorder", &popup_border, 0)) style.PopupBorderSize = popup_border ? 1.0f : 0.0f; }
    ...
    if (ui::BeginTabBar("##tabs", ImGuiTabBarFlags_None))
    {
        ...
        if (ui::BeginTabItem("Colors"))
        {
            ...
            if (ui::Button("Export Unsaved"))
            {
                if (output_dest == 0)
                    ...
                    ui::LogToTTY();
                ui::LogText("ImVec4* colors = ui::GetStyle().Colors;" IM_NEWLINE);
            ...
            ui::SameLine(); ui::Checkbox("Only Modified Colors", &output_only_modified, 0);
        ...
        {
            ui::Checkbox("Anti-aliased lines", &style.AntiAliasedLines, 0); ui::SameLine(); ShowHelpMarker("When disabling anti-aliasing lines, you'll probably want to disable borders in your style as well.");
            ui::Checkbox("Anti-aliased fill", &style.AntiAliasedFill, 0);
            ui::PushItemWidth(100);
...
struct ExampleAppConsole
{
    ...
    void    Draw(const char* title, bool* p_open)
    {
        ...
        {
            if (ui::Checkbox("Auto-scroll", &AutoScroll, 0))
                if (AutoScroll)
                    ScrollToBottom = true;
            ui::EndPopup();
        }
        ...
        // Display every line as a separate entry so we can change their color or add custom widgets. If you only want raw text you can use ui::TextUnformatted(log.begin(), log.end());
...
static void ShowExampleAppPropertyEditor(bool* p_open)
{
    ...
    ShowHelpMarker("This example shows how you may implement a property editor using two columns.\nAll objects/fields data are dummies here.\nRemember that in many simple cases, you can use ui::SameLine(xxx) to position\nyour cursor horizontally instead of using the Columns() API.");
...
static void ShowExampleAppConstrainedResize(bool* p_open)
{
    ...
    if (ui::Begin("Example: Constrained Resize", p_open, flags))
    {
        ...
        ui::PopItemWidth();
        ui::Checkbox("Auto-resize", &auto_resize, 0);
...
void ShowExampleAppDocuments(bool* p_open)
{
    ...
    for (int doc_n = 0; doc_n < app.Documents.Size; doc_n++)
    {
        ...
        if (doc_n > 0)
            ui::SameLine();
        ui::PushID(doc);
        if (ui::Checkbox(doc->Name, &doc->Open, 0))
            if (!doc->Open)
                doc->DoForceClose();
        ui::PopID();
    }
...
void ui::ShowUserGuide() {}
void ui::ShowStyleEditor(ImGuiStyle*) {}
